...
<div class="item"><div class="itemName">클릭 더블</div><div class="itemDesc">업그레이드해도 항상 ×2 고정</div><div class="itemRow"><span class="level" id="doubleLevel">Lv.0 · ×1</span><button class="action" id="buyDouble" type="button"></button></div></div>
<div class="item"><div class="itemName">인생역전</div><div class="itemDesc">5,000원으로 5회 추첨 · 1등 0.2% / 2등 0.8% / 3등 1%</div><div class="itemRow"><span class="level">5회 묶음</span><button class="action" id="buyLottery" type="button">5,000원</button></div><div id="lotteryResult"></div></div>
...
function render(){
 ...
 $("stageTitle").textContent=s.name;$("money").textContent=fmt(money);$("clickIncome").textContent="터치 +"+fmt(gainPerTouch());
...
function lottery(){
 ...
 for(let i=0;i<5;i++){const r=Math.random()*100;if(r<.2){total+=500000000;results.push("1등 +5억")}else if(r<1){total+=5000000;results.push("2등 +500만원")}else if(r<2){total+=10000;results.push("3등 +1만원")}else results.push("꽝")}
